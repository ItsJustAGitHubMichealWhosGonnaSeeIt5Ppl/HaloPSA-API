...
def create_dataclass(import_rules:dict, child_schema:child_schema_types="ignore", create_list:bool=False):
    ...
    schema_name:str =  import_rules["schema"]
    property_rules:dict= import_rules["property_rules"]
    
    if json_schemas[schema_name]['type'] != "object":
        raise ValueError(f"Unexpected schema type {json_schemas[schema_name]['type']}")
    
    if "class_name" in import_rules.keys() and  import_rules['class_name'] != "":
        class_name:str = import_rules['class_name']
    else:
    ...
    if 'additional_properties' in import_rules: # add additional rules
        schema_properties.update({k:v for k,v in import_rules['additional_properties'].items()})
...
schemas_to_import_HELPER = [
    {
        "schema": "InvoiceHeader",
        "class_name":"Invoice", # Optional
        "property_rules":# Fields that need "field(default=None)" added (AKA they arent always returned) or other rules/tweaks
            {
                "invoice_date":{"format": "%Y-%m-%dT%H:%M:%S"},
                "schedule_date":{"format": "%Y-%m-%dT%H:%M:%S"},
                "datesent":{"format": "%Y-%m-%dT%H:%M:%S"},
                "dateposted":{"format": "%Y-%m-%dT%H:%M:%S.%f"},
                "last_synced":{"format": "%Y-%m-%dT%H:%M:%S.%f"},
                "last_modified":{"format": "%Y-%m-%dT%H:%M:%S.%f"},
                "currency_code_name": {"default": None},
                "disabled": {"default": None},
                "accountsid": {"default": None},
                "address1": {"default": None},
                "address2": {"default": None},
                "address3": {"default": None},
                "address4": {"default": None},
                "address5": {"default": None},
                "deladdress1": {"default": None},
                "deladdress2": {"default": None},
                "deladdress3": {"default": None},
                "deladdress4": {"default": None},
                "deladdress5": {"default": None},
                "tel_number": {"default": None},
                "contactname": {"default": None},
                "global_tax_code": {"default": None},
                "notes_1": {"default": None},
                "notes_2": {"default": None},
                "notes_3": {"default": None}
            }
    }
]
